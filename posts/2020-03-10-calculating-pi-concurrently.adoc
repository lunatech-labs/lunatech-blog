...
If we set the size of the ForkJoinPool to 1, 2, 4, 8, 12, 16, 32 we get the
following values for the calculation time (in seconds) respectively: 12.12,
6.23, 3.03, 1.78, 1.34, 1.36, 1.43
...
With this, we come to the conclusion of this article. We have implemented a CPU-bound computation and managed to exploit our computer's multi-processing (or multi-core processing) capabilities using two very different approaches. First we used Scala's `Future` API followed by an Akka Streams based approach.
