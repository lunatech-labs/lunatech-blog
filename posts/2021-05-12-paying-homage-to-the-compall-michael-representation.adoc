...
However, in practice allocations can still happen in a number of situations. For example, imagine that we decide to allow the `toKm` method to accept values of a super-type `Distance` (of which `Kilometers` and `Miles` would be two sub-types), we could have code like the following.
...
This code is perfectly fine, but if you had chosen the `AnyVal` route to avoid allocations, then you would be disappointed to discover that now with the supertype-subtype relationship you will once again have allocations when you pass a value of `Kilometers` or `Miles` to the function `toKm`.
...
- abstract `def` declarations that allow us to do useful things with our two types of unit, in this case a method `add` for adding two `Kilometers` values and a method `toKm` for converting a `Miles` value to a `Kilometers` value.
