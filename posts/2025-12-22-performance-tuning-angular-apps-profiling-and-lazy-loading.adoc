...
`ChangeDetectionStrategy.OnPush` tells Angular to skip checks unless:

- An input reference changes
- An event handler runs in the component
...
OnPush shines when inputs are immutable, or when the template binds to observables rather than mutable objects.
...
A better approach is to compute outside the template, but still keep it correct when inputs change. For example, if `total` depends on inputs:
...
The key point is not just caching, but updating the cached value when the underlying data changes.

==== Tracking List Items

Without `trackBy`, Angular may remove and recreate DOM nodes whenever a list updates.
...
- Use OnPush where it fits the data flow
- Move computations out of templates and keep them updated
...
- Switch key components to OnPush and confirm the data flow still behaves.
