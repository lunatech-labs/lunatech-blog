...
Understand Angular’s change detection mechanism and how to optimize performance using OnPush strategy. Get an introduction to using Signals for more granular reactivity. This article gives you a practical mental model, then shows how to apply it with Zone.js, OnPush, and signals—plus a path to zoneless apps when you’re ready.
...
[source,typescript]
----
import { Component, NgZone, ChangeDetectorRef, ChangeDetectionStrategy, signal, inject } from '@angular/core';
...
export class CpuHeavyComponent {
  private zone = inject(NgZone);
  private cdr = inject(ChangeDetectorRef);

  // Store state as a signal for granular updates
  ...
  ngOnInit() {
    this.zone.runOutsideAngular(() => {
      ...
      const id = setInterval(() => {
        // Only re-enter Angular when UI needs to change.
        this.zone.run(() => {
          this._frames.set(++n);
          // Signals schedule updates automatically. If not using signals, call:
          // this.cdr.markForCheck();
        });
        if (n > 3000) { clearInterval(id); }
      }, 16);
    });
  }
}
