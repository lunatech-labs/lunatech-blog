...
But unlike suggested by their name, e-voting schemes are not only digitalized version of traditional voting systems where paper ballots are counted by hand.
...
But this does not prevent legitimate voters from colluding and potentially learning unexpected information, as we will see <<attack, later>>. 
