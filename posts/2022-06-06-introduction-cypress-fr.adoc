...
Ce genre de test ne se soucie pas de l’implémentation interne de notre produit, mais va adopter le point de vue d’un utilisateur pour vérifier le comportement général de notre produit.
...
Après avoir sauvegardé, nous voyons le résultat directement dans l’interface graphique :

image::blog-loaded.png[Test vérifiant si les aperçus sont bien affichés, width = 700]

En bas de la partie gauche, vous pouvez voir `expected 36 to be above 0` comme nous voulions.
