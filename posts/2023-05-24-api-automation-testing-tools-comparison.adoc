...
=== Comparison Table
.comparison table for the above-mentioned tools
image::comparison table.png[alt="Figure 1: comparison table for the above mentioned tools"]

=== Conclusion

In conclusion, there are many API testing tools available that offer a range of features and functionalities. Each tool has its strengths and weaknesses, catering to different requirements and preferences. With Lunagraph, it is crucial to consider that app is only single API integrated with internal application and has no GUI. After evaluating the aforementioned tools, we decided to go for *Postman*, as it was easy to use and had great integration and reporting capabilities.
