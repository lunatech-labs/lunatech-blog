= Semantic Kernel API and Quarkus
iarmush
v1.0, 2024-01-10
...
== About what is it?
The goal is to create a simple microservice that uses the Semantic Kernel API to correct grammatical, punctuation, and logical mistakes in the input text by invoking OpenAI. We need the frontend application with an input field, checkboxes, and radio buttons for configuring additional features, and an output field. And the backend application for using invoking OpenAI.
