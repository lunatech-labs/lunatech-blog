...
* The `Pinger` actor defines an internal behavior of type `Behavior[CommandIncludingResponses]`
* The `Behavior[Command]` is derived from the `Behavior[CommandIncludingResponses]` by applying the `narrow` method on the latter
...
Admittedly, some magic has happened here and the question to ask is, _What is this `narrow` method doing?_. Let's look at that in the next section.
