...
tags:
  - dotty
  - akka
img: images/media/2020-02-12-using-dotty-union-types-with-akka-typed/background.png
---
= Using Dotty Union types with Akka Typed

:imagesdir: /images/media/2020-02-12-using-dotty-union-types-with-akka-typed
...
----
<1> Let's explain this step-by-step: to start, we extend the `Command` ADT by adding a new message that wraps the response.
<2> Next, we use the `messageAdaptor` method on the `ActorContext` to create a message adapter.
<3> The type of the latter is an `ActorRef[PingPong.Response]`. The message adapter is sent along to the `PingPong` actor as a field in the `Ping` message.
...
----
<1> We introduce a type alias for convenience.
<2> It is a Union of the original `Command` ADT and the type of the responses we want to be able process. We use this Union type when we describe our `Behavior`.
