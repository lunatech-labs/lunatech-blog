...
<style>
  ...
  /* The theme draws the byline's dot separators with `.author span`, which also
     catches the multi-author byline, since that branch of the template emits a
     <span> rather than an <a>. Without this the names render inside a 4px box. */
  .author span.author__text {
    display: inline-flex; align-items: center; white-space: nowrap;
    background: none; height: auto; width: auto;
    border-radius: 0; margin: 0; opacity: 1;
  }

  /* A two-author byline gets the theme's generic avatar, because the layout only
     builds a GitHub one for a single handle. Both faces belong here, and doing it
     from this post's own stylesheet leaves every other post alone. */
  .author__avatar-lg { content: url(https://github.com/LSRaGeUx.png); }

  .author span.author__text::before {
    content: "";
    ...
    background: url(https://github.com/me-no-code.png) center / cover, var(--pearl);
  }
</style>
++++
